...
use byte_board::{
    bitboard::Bitboard,
    definitions::{NumberOf, BISHOP_BLOCKER_PERMUTATIONS, EMPTY, ROOK_BLOCKER_PERMUTATIONS},
    magics::MagicNumber,
...
fn find_magic_numbers(piece: Piece) -> Vec<MagicNumber> {
    ...
    for sq in 0..1 {
        ...
        let end = total_permutations - 1u64;
        ...
        while !found {
            magic.magic_value = rng.gen::<u64>() & rng.gen::<u64>() & rng.gen::<u64>();
            // let's be optimistic
            found = true;
            for (i, bb) in blocker_bitboards.iter().enumerate() {
                let index = magic.index(bb);
                ...
                if table[index] == Bitboard::default() {
                    assert!(
                        index as u64 >= offset && index as u64 <= total_permutations - 1,
                        "index out of bounds"
                    );
                    ...
                    // First we clean up
                    for i in offset..end {
                        table[i as usize] = Bitboard::default();
                    }
                    found = false;
                    break;
                }
